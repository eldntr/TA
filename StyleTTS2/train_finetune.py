...
def main(config_path):
    ...
    peft_enabled = config.get('peft_enabled', False)
    peft_trainable_modules = config.get('peft_trainable_modules', [])
    ...
    if peft_enabled:
        freeze_all(model)
        if peft_trainable_modules:
            unfreeze_modules_by_name(model, peft_trainable_modules)
        else:
            unfreeze_lpep(model)
            unfreeze_ppim(model)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if not torch.isfinite(g_loss):
                raise RuntimeError(
                    "Non-finite g_loss in train_finetune: "
                    f"g_loss={g_loss.item()}, "
                    f"loss_mel={loss_mel.item()}, "
                    f"loss_F0_rec={loss_F0_rec.item()}, "
                    f"loss_ce={loss_ce.item()}, "
                    f"loss_norm_rec={loss_norm_rec.item()}, "
                    f"loss_dur={loss_dur.item()}, "
                    f"loss_gen_all={loss_gen_all.item()}, "
                    f"loss_lm={loss_lm.item()}, "
